...
- [x] 26.1 Create `src/adapters/driving/http/launch.ts` — `POST /launch` route wiring the launcher use case. `launch.ts` parses/validates the JSON body and calls the thin `application/launch-agent-session/launch-agent-session.ts` use case; wired into `stream.ts`'s `createStreamServer(hub, launcher?)` (backward-compatible optional param) and composed in `server.ts` alongside the four ingestion adapters.
- [x] 26.2 Add launcher UI control to `ui/components/` (organism) wired through `OfficeContainer`. `launch-control.ts` (pure view: the four supported targets) + `OfficeContainer.requestLaunch` (a new injected `LaunchClient` seam, mirroring the existing `StreamConnection`/`StreamConnectionFactory` pattern) + `FetchLaunchClient` (browser `fetch` adapter) + thin untested `main.ts`/`index.html` DOM wiring (one button per target), matching the established main.ts convention.
- [x] 26.3 RED+GREEN (dependency-cruiser rule extension): launcher module has no import edge to any `adapters/driven/{claude-code,codex,opencode,antigravity}` tailer/parser file (Subsystem Separation from Ingestion). Rule `launcher-no-ingestion-adapter-imports` added to `.dependency-cruiser.cjs`; `test/dependency-cruiser/launcher-boundary.test.ts` proves it fails on a deliberate violation fixture and passes on a clean one, mirroring the existing domain/pixi boundary test pattern. Real `npm run lint:deps`: 0 violations (157 modules, 469 dependencies cruised).
- [x] 26.4 RED+GREEN: modifying the launcher's spawn mechanism leaves all four adapters' existing fixture suites unaffected. Verified by the full `npm test` run (449/449 passing, including every Claude Code/Codex/Antigravity/OpenCode adapter and fixture suite) after all Phase 23-26 launcher changes landed — none of those suites needed any change.

### Phase 27: Slice 5 Verification (full-system)

- [x] 27.1 Run `npm test` — launcher, correlation, and full regression suite (all prior slices) green. **449/449 passing** (68 test files), `npm run typecheck` 0 errors, `npm run lint:deps` 0 violations (157 modules, 469 dependencies cruised).
- [x] 27.2 Manual smoke: launch one real harness CLI from the scene; diff the resulting argv against a manual invocation (byte-identical). Ran the REAL `ChildProcessSessionLauncher` (not a fake) via a throwaway script, launching `claude --version` — an inert, instantly-exiting invocation, deliberately used INSTEAD of a bare `claude` launch (which would start a real, long-lived interactive session — unsafe per this work unit's runtime-safety constraint). Result: `buildLaunchCommand` argv `["claude","--version"]`, resolved binary path `/Users/andresalvarez/.local/bin/claude` — byte-identical to `which claude` (manual resolution) and to typing `claude --version` (manual invocation). The real spawn produced pid 218, which exited cleanly on its own (confirmed gone via `ps -p 218` afterward, zero orphans). `launch_requested` then `launch_started` were emitted with the exact argv/binaryPath/cwd/pid, verified above. Separately confirmed via the full composed server + a real `POST /launch` HTTP call (pid 369, also confirmed exited, zero orphans). Success criterion #4's PURE zero-arg case (`args: []` -> `["claude"]`, no flags at all) is covered by the `buildLaunchCommand` unit tests (task 23.2) rather than a live spawn, since actually running bare `claude` would start an interactive session this work unit must not leave running. Codex's `--version` was observed to hang indefinitely in this environment during an initial exploratory check (killed safely, zero orphans left) — Codex was therefore deliberately NOT live-spawned for this smoke; its argv/PATH-resolution logic is covered by existing unit tests only, and this is recorded as a scope decision, not a silent gap.
- [x] 27.3 Manual smoke: confirm zero writes to any of `~/.claude/`, `~/.codex/`, `~/.local/share/opencode/`, `~/.gemini/` config/hook/db/transcript paths across a full launch + ingest + animate cycle. Ran the REAL composed `src/server.ts` (all four `ActivitySource`s + the launcher wired together, exactly as in production) for several seconds against the real, machine-specific harness trees, issued a real `POST /launch` for `claude-code`, then shut the server down cleanly (confirmed zero orphan processes). Before/after evidence: `~/.codex` (6192/6192 files, size byte-identical), `~/.local/share/opencode` directory (100/100 files, size byte-identical), `~/.gemini` (1599/1599 files, size byte-identical) — all THREE completely untouched. `~/.claude` file count unchanged (4849/4849); the two files that changed (`~/.claude/backups/.claude.json.backup.*` and this very agent's own live subagent transcript under `~/.claude/projects/.../subagents/`) are both attributable to processes this server/launcher does not touch, matching the identical pattern already documented in the `browser-entrypoint` work unit (task 31.3). `opencode.db`/`opencode.db-wal`: byte-identical sha256 before/after. `opencode.db-shm`: identical sha256 (content unchanged) and size; only its mtime changed — the same documented, accepted SQLite WAL-reader-coordination behavior from task 17.6's correction (a genuinely read-only connection still touches `-shm`'s timestamp, never its committed content).
