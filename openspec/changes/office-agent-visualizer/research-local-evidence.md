...
## Addendum — Antigravity `mem_save` record shape (captured by the orchestrator)

The research lanes could not observe an Engram `mem_save` in any Antigravity transcript. The cause
was identified and closed rather than left as inference.

### Root cause of the gap

`~/.gemini/antigravity-cli/mcp_config.json` is a STALE, UNUSED file. The live configuration that
`agy` actually reads is `~/.gemini/config/mcp_config.json`. Before this probe, `agy mcp list`
returned only `codegraph` and `graphify`, exactly matching the live file — Engram was never wired
into Antigravity, so no `mem_save` could ever have appeared in its logs.

Adapter implication: never read `~/.gemini/antigravity-cli/mcp_config.json` to discover which MCP
servers a session had available. Use `~/.gemini/config/mcp_config.json` (global) or the workspace
`.agents/mcp_config.json`.

### Method

With the user's explicit approval, `engram` was added to `~/.gemini/config/mcp_config.json`
(additive, reversible), confirmed present via `agy mcp list`, and one disposable non-interactive
session was run:

```
agy --dangerously-skip-permissions --print-timeout 3m --print='<prompt instructing a single mem_save call>'
```

The new conversation appeared at
`~/.gemini/antigravity-cli/brain/692e6e50-7c09-4732-9a9b-4353b7742ca4/`.

### VERIFIED record shape

From `.../.system_generated/logs/transcript.jsonl`, record at `step_index: 1`,
`type: "PLANNER_RESPONSE"`, `source: "MODEL"`, `status: "DONE"`:

```json
{"name": "call_mcp_tool",
 "args": {
   "Arguments": "{\"capture_prompt\":false,\"content\":\"...\",\"project\":\"pixel-agents\",\"title\":\"antigravity-mcp-probe\",\"type\":\"discovery\"}",
   "ServerName": "\"engram\"",
   "ToolName": "\"mem_save\"",
...
1. Every MCP tool call is funnelled through a single tool named `call_mcp_tool`. The MCP server and
   tool identity live in `args.ServerName` and `args.ToolName`, NOT in `tool_calls[].name`. Matching
   on `tool_calls[].name` alone can never detect an Engram write on this harness.
2. `ServerName`, `ToolName`, `toolAction` and `toolSummary` are DOUBLE-ENCODED: the value of
   `ServerName` is the six-character string `"engram"` including its literal surrounding quotes, not
   `engram`. The adapter must strip one JSON layer before comparing. A naive equality check fails.
3. `args.Arguments` is a JSON-encoded STRING, not a nested object; it needs a second parse.
4. `toolAction` and `toolSummary` are human-readable labels generated by the harness and are
   directly usable as office-scene captions.
5. The logs directory also contains `chunks/transcript/*.jsonl` and `chunks/transcript_full/*.jsonl`
   mirrors, which matter when choosing what the tailer follows.

### Requirement #6 evidence status — now complete for all four harnesses

| Harness | Detection rule | Status |
|---|---|---|
| Claude Code | `tool_use` record with `name` matching `mcp__*engram*__mem_save` | VERIFIED |
| Codex | `event_msg` / `item_completed` with `payload.item.type: "McpToolCall"` | VERIFIED |
| OpenCode | `part.tool` column equal to `engram_mem_save` (`<server>_<tool>`, no `mcp__` prefix) | VERIFIED |
| Antigravity | `tool_calls[].name == "call_mcp_tool"` with de-quoted `args.ServerName == "engram"` and `args.ToolName == "mem_save"` | VERIFIED |

**Design consequence:** there is no single cross-harness naming convention for an MCP tool call.
Each of the four harnesses names, nests and encodes it differently. The `memory_write` normalized
event therefore requires four distinct detector implementations behind one interface, not a shared
pattern match.
